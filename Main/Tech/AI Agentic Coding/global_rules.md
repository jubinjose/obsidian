...
- **Health:** App must have a GET /health endpoint that returns a JSON object containing the current ISO timestamp
...
- **`@fastify/formbody` must be v8+ for Fastify 5**: v7 is Fastify 4 only — `npm i @fastify/formbody@^8`.